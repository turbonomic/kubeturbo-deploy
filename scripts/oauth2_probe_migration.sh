...
check_deployment_status() {
    ...
    local targetReplicas
    targetReplicas=$(kubectl -n $namespace get deployment $deploy_name -o=jsonpath='{.spec.replicas}')
    local readyReplicas
    readyReplicas=$(kubectl -n $namespace get deployment $deploy_name -o=jsonpath='{.status.readyReplicas}') && readyReplicas=${readyReplicas:-"0"}
